...
try {
  ...
  if (layout.documentWidth > layout.width) throw new Error('Phone layout overflows the document');
  if (!layout.text.includes('No physiology model is admitted')) throw new Error('Scope limitation missing');
  ...
  for (const link of layout.links.filter(link => !link.startsWith('#'))) {
    if (!/^[a-z-]+\.(json|csv|pyz)$/u.test(link)) throw new Error('Unexpected evidence link target');
  ...
  if (semantics.language !== 'en' || semantics.mains !== 1 || semantics.headerScopes.some(x=>x!=='col')) {
  ...
  await writeFile(resolve(output, 'browser-check.json'), JSON.stringify({
    ...
    semantics, links: layout.links, evidenceLinkFilesExist: true, externalRequests: 0,
