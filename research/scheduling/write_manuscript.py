#!/usr/bin/env python3
"""Format frozen evidence without executing the platform. AGPL-3.0-or-later."""
import json
from pathlib import Path
p=Path(__file__).resolve().parent;c=p/'confirmation';pairs=json.loads((c/'paired-effects.json').read_text());results=json.loads((c/'results.json').read_text());prov=json.loads((c/'provenance.json').read_text());cost=json.loads((c/'costs.json').read_text())
table='| Scenario | Seed | Baseline mean wait (s) | Delayed mean wait (s) | Δ mean (s) | Δ p95 (s) | Δ makespan (s) |\n|---|---:|---:|---:|---:|---:|---:|\n'
for r in pairs:table+=f'| {r["kind"]} | {r["seed"]} | {r["baseline_mean_wait_s"]:.3f} | {r["delayed_mean_wait_s"]:.3f} | {r["delta_mean_wait_s"]:.3f} | {r["delta_p95_wait_s"]:.3f} | {r["delta_makespan_s"]:.3f} |\n'
text=f'''# Finite service-delay amplification in synthetic FCFS queues
...
Measured driver wall time was {cost['total_driver_wall_s']:.3f} seconds, summed CLI run wall time {cost['sum_cli_run_wall_s']:.3f} seconds, and summed platform-reported worker lifecycle wall time {cost['sum_worker_wall_s']:.3f} seconds. Those are nested timings and must not be added. The stored worker timing includes subprocess management and waiting, not isolated scheduling CPU time. Evidence occupied {cost['artifact_bytes']:,} bytes at the end-of-analysis measurement before final cost/figure/manuscript files. CPU-seconds, peak memory and financial cost were not instrumented; no paid resources were requested. These host-specific startup-dominated measurements are not throughput benchmarks.
